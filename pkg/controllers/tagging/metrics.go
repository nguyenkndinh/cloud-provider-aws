...
var (
	...
	workItemErrorCounter = metrics.NewCounterVec(
		&metrics.CounterOpts{
			Name:           "tagging_workitem_latency",
			Help:           "workitem error count",
			StabilityLevel: metrics.ALPHA,
		},
		[]string{"workqueue"})
)
...
func registerMetrics() {
	register.Do(func() {
		...
		legacyregistry.MustRegister(workItemErrorCounter)
	})
}

func recordWorkitemMetrics(actionName string, timeTaken float64, err error) {
	if err != nil {
		workItemErrorCounter.With(metrics.Labels{"workqueue": actionName}).Inc()
	}
